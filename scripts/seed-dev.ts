...
export async function getDashboardData(): Promise<DashboardData> {
  ...
  const [leadsTodayRes, allLeadsRes, devisRes, demandesRes, relancesUrgentRes] =
    await Promise.all([
      supabase
        .from("leads")
      ...
      supabase
        .from("devis")
        .select(
          "statut, montant_ttc, coeff_saisonnalite, coeff_capacite, coeff_delai",
        ),
  ...
  if (leadsTodayRes.error)
    throw new Error(`Erreur leadsToday: ${leadsTodayRes.error.message}`);
  if (allLeadsRes.error)
    throw new Error(`Erreur allLeads: ${allLeadsRes.error.message}`);
  if (devisRes.error)
    throw new Error(`Erreur devis: ${devisRes.error.message}`);
  if (demandesRes.error)
    throw new Error(`Erreur demandes: ${demandesRes.error.message}`);
  if (relancesUrgentRes.error)
    throw new Error(
      `Erreur relancesUrgent: ${relancesUrgentRes.error.message}`,
    );

  const totalLeads = allLeadsRes.count || 0;
  const totalDevis = devisRes.data?.length || 0;
  const devisAcceptes =
    devisRes.data?.filter((d) => d.statut === "accepte") || [];
  ...
  const avgCompletude =
    totalDemandes > 0
      ? Math.round(
          demandesRes.data.reduce(
            (acc, curr) => acc + Number(curr.score_completude || 0),
            0,
          ) / totalDemandes,
        )
      : 0;
  ...
  const escaladeCount =
    demandesRes.data?.filter((d) => d.type_statut === "escalade").length || 0;
  const hitlRate =
    totalDemandes > 0 ? Math.round((escaladeCount / totalDemandes) * 100) : 0;

  // Construction des données du Funnel Commercial (Recharts)
  const countStatutLeads = (statut: string) =>
  ...
  const funnelData = [
    { name: "1. Nouveaux Leads", value: totalLeads, fill: "#4A5568" },
    {
      name: "2. Qualifiés (IA)",
      value:
        countStatutLeads("qualifie") +
        countStatutLeads("en_attente") +
        countStatutLeads("escalade"),
      fill: "#319795",
    },
